...
namespace PKOModelViewer
{
    ...
    public class GltfExporter
    {
        ...
        private static int GetActualBoneCount(lwGeomObjInfo geom, lwAnimDataBone boneData)
        {
            ...
            // CRITICAL FIX: First, determine how many valid matrices we actually have
            // This prevents the Blender IndexError by ensuring we never claim more bones than we can provide matrices for
            ...
            // CRITICAL FIX: Never exceed the number of valid matrices we actually have
            // This is the root cause of the Blender "IndexError: list index out of range"
            if (validMatrixCount > 0)
            {
                actualBoneCount = Math.Min(actualBoneCount, validMatrixCount);
            }
            else if (actualBoneCount > 0)
            {
                // ENHANCED: If we have bone data but no valid matrices, still export skeleton with identity matrices
                // This allows PKO characters to retain their skeletal structure even with missing matrix data
                System.Diagnostics.Debug.WriteLine($"WARNING: Character has {actualBoneCount} bones but no valid matrices - using identity matrices");
            }
            
            // Enhanced skeletal data detection - preserve bone structure when possible
            if (actualBoneCount == 0 && (geom.mesh.blend_seq != null && geom.mesh.blend_seq.Length > 0))
            {
                // Only fallback to static if we truly have no bone structure
                if (boneData != null && boneData._header.bone_num > 0)
                {
                    actualBoneCount = Math.Min((int)boneData._header.bone_num, validMatrixCount > 0 ? validMatrixCount : (int)boneData._header.bone_num);
                }
                else
                {
                    actualBoneCount = validMatrixCount > 0 ? 1 : 0;
                }
            }
        ...
        private static void CreateCombinedCharacterGltf(List<lwGeomObjInfo> characterParts, List<string> partNames, string outputPath, string characterName, lwAnimDataBone boneData)
        {
            ...
            if (boneData != null && characterParts.Count > 1)
            {
                ...
                try
                {
                    ...
                    ExportToGltf(unifiedGeom, outputPath, characterName + "_unified", null, boneData);
            ...
            if (characterParts.Count == 1)
            {
                ...
                ExportToGltf(characterParts[0], outputPath, characterName, null, boneData);
                return;
            }
            ...
            for (int i = 0; i < characterParts.Count; i++)
            {
                try
                {
                    ...
                    ExportToGltf(characterParts[i], partOutputPath, $"{characterName}_{partName}", null, boneData);
        ...
        private static lwGeomObjInfo CreateUnifiedGeometry(List<lwGeomObjInfo> characterParts, lwAnimDataBone boneData)
        {
            ...
            foreach (var part in characterParts)
            {
                ...
                if (part != null && part.mtl_seq != null)
            ...
            unified.mesh.vertex_seq = new Mindpower.D3DXVECTOR3[totalVertices];
            if (totalIndices > 0)
                unified.mesh.index_seq = new uint[totalIndices];
            if (totalMaterials > 0)
                unified.mtl_seq = new Mindpower.lwMtlTexInfo[totalMaterials];
            
            // Combine skeletal data
            var allBlendData = new List<Mindpower.lwBlendInfo>();
            ...
            foreach (var part in characterParts)
            {
                if (part == null || part.mesh.vertex_seq == null) continue;
        ...
        private static void CreateManualGltfWithStrategy(lwGeomObjInfo geom, string outputPath, string modelName, string modelPath, lwAnimDataBone boneData, UVStrategy uvStrategy)
        {
            ...
            try
            {
                ...
                // Check for skeletal mesh data - ENHANCED to support identity matrix fallback
                bool hasSkeletalData = (geom.mesh.blend_seq != null && geom.mesh.blend_seq.Length > 0);
                bool hasBoneStructure = (boneData != null && boneData._header.bone_num > 0);
                int actualBoneCount = GetActualBoneCount(geom, boneData);
                
                // ENHANCED FIX: Export skeleton if we have ANY skeletal indicators, use identity matrices when needed
                bool exportSkeleton = (hasSkeletalData || hasBoneStructure) && actualBoneCount > 0 && boneData != null;
                
                // Debug information for skeleton export decision
                System.Diagnostics.Debug.WriteLine($"SKELETON EXPORT DECISION:");
                System.Diagnostics.Debug.WriteLine($"  - hasSkeletalData: {hasSkeletalData}");
                System.Diagnostics.Debug.WriteLine($"  - hasBoneStructure: {hasBoneStructure}");
                System.Diagnostics.Debug.WriteLine($"  - actualBoneCount: {actualBoneCount}");
                System.Diagnostics.Debug.WriteLine($"  - boneData present: {boneData != null}");
                System.Diagnostics.Debug.WriteLine($"  - EXPORT SKELETON: {exportSkeleton}");
        ...
        private static void CreateSkinsSection(StringBuilder sb, lwGeomObjInfo geom, string modelName, lwAnimDataBone boneData)
        {
            ...
            // ENHANCED: Always add inverse bind matrices for skeletal meshes, use identity when missing
            // This ensures consistent GLTF structure for PKO characters
            bool hasValidMatrices = boneData != null && boneData._invmat_seq != null && boneData._invmat_seq.Length > 0;
            bool hasInvalidButPresentMatrices = false;
            
            if (hasValidMatrices)
            {
                // Check if matrices are actually valid
                for (int i = 0; i < Math.Min(actualBoneCount, boneData._invmat_seq.Length); i++)
                {
                    if (boneData._invmat_seq[i].m == null || boneData._invmat_seq[i].m.Length < 16)
                    {
                        hasInvalidButPresentMatrices = true;
                        break;
                    }
                }
            }
            
            // Always export inverse bind matrices for skeletal characters (use identity when needed)
            if (actualBoneCount > 0)
            {
                ...
                // Debug information
                if (!hasValidMatrices || hasInvalidButPresentMatrices)
                {
                    System.Diagnostics.Debug.WriteLine("WARNING: Using identity matrices for inverse bind matrices due to missing/invalid PKO matrix data");
                }
            }
        ...
        private static void CreateAccessorsAndBufferViews(StringBuilder sb, lwGeomObjInfo geom, lwAnimDataBone boneData, bool exportSkeleton, UVStrategy uvStrategy, string outputPath)
        {
            ...
            // Inverse bind matrices accessor for skeletal mesh - ENHANCED to always include for skeletons
            if (exportSkeleton && boneData != null)
            {
                sb.AppendLine("    ,{");
                sb.AppendLine("      \"bufferView\": " + bufferViewIndex + ",");
                sb.AppendLine("      \"componentType\": 5126,");
        ...
        private static void CreateBufferViews(StringBuilder sb, lwGeomObjInfo geom, lwAnimDataBone boneData, bool exportSkeleton, int totalViews)
        {
            ...
            // ENHANCED: Always create buffer view for skeletal characters (use identity when PKO data missing)
            if (exportSkeleton && boneData != null)
            {
        ...
        private static int CalculateBufferSize(lwGeomObjInfo geom, lwAnimDataBone boneData, bool exportSkeleton)
        {
            ...
            // ENHANCED: Always include matrix space for skeletal characters (identity when PKO data missing)
            if (exportSkeleton && boneData != null)
        ...
        private static void CreateBinaryDataWithStrategy(lwGeomObjInfo geom, string binFilePath, lwAnimDataBone boneData, bool exportSkeleton, UVStrategy uvStrategy)
        {
            ...
            using (var writer = new BinaryWriter(stream))
            {
                ...
                // Write inverse bind matrices for skeletal mesh - ENHANCED to always write when skeleton present
                // CRITICAL ENHANCEMENT: Always write matrices for skeletal characters, use identity when PKO data missing
                if (exportSkeleton && boneData != null)
                {
                    long matricesStartPos = stream.Position;
                    int actualBoneCount = GetActualBoneCount(geom, boneData);
                    
                    bool hasValidMatrixData = boneData._invmat_seq != null && boneData._invmat_seq.Length > 0;
                    
                    for (int i = 0; i < actualBoneCount; i++)
                    {
                        bool useIdentity = true;
                        
                        // Try to use PKO's actual inverse bind matrices if available with EXTRA validation
                        if (hasValidMatrixData && 
                            i < boneData._invmat_seq.Length && 
                        ...
                        // Fallback to identity matrix for missing, invalid, or insufficient PKO matrix data
                        if (useIdentity)
                        {
                            // Write GLTF-compliant identity matrix (column-major order)
                            // This ensures skeletal structure is preserved even without PKO matrix data
                    ...
                    if (!hasValidMatrixData)
                    {
                        debugLog += $"WARNING: Used {actualBoneCount} identity matrices due to missing PKO matrix data\n";
                    }
                }
